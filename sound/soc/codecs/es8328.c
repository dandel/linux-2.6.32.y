...
#else
	es8328_dapm_event(codec, codec->dapm_state);
#endif
}
...
#else
	es8328_dapm_event(codec, SNDRV_CTL_POWER_D3cold);
#endif
	return 0;
}
...
static int es8328_init(struct snd_soc_device *socdev)
{
...
#else
	codec->dapm_event = es8328_dapm_event;
#endif
	...
	codec->reg_cache =
			kzalloc(sizeof(u16) * ARRAY_SIZE(es8328_reg), GFP_KERNEL);
	if (codec->reg_cache == NULL)
		return -ENOMEM;
	memcpy(codec->reg_cache, es8328_reg,
		sizeof(u16) * ARRAY_SIZE(es8328_reg));
	codec->reg_cache_size = sizeof(u16) * ARRAY_SIZE(es8328_reg);
#else// XXX: mhfan
...
#else
	es8328_dapm_event(codec, SNDRV_CTL_POWER_D1);	// XXX:
#endif
	...
	es8328_write(codec, ES8328_ADCCONTROL1,  0x00);	// 0x09
	es8328_write(codec, ES8328_ADCCONTROL2,  0x50); //0x0a
	es8328_write(codec, ES8328_ADCCONTROL3,  0x02);  //0x0b
	...
	es8328_write(codec, ES8328_ADCCONTROL10,  0xf2);//0x12

	
	es8328_write(codec, ES8328_ADCCONTROL11, 0xb0); //0x13
	es8328_write(codec, ES8328_ADCCONTROL12, 0x05); //0x14
	es8328_write(codec, ES8328_ADCCONTROL13, 0x06); //0x15
	es8328_write(codec, ES8328_ADCCONTROL14, 0x7b); //0x16
	...
	es8328_write(codec, ES8328_DACCONTROL4, 0x00); //0x1a
	es8328_write(codec, ES8328_DACCONTROL5, 0x00); //0x1b
	...
	es8328_write(codec, ES8328_DACCONTROL24, 0x1c); //0x2e
	es8328_write(codec, ES8328_DACCONTROL25, 0x1c); //0x2f
