...
//	writel(rI2S_PRESCALER_value, imapx200_i2s.regs + rI2S_PRESCALER);
	...
	if (freq == 8000)
	{
	imapx200_get_pll((0x1<<31|0x1<<12 | 0x0<<7 |0x23 ), 12000000);
	__raw_writel((0x1<<31 |0x1<<12| 0x0<<7 | 0x23), rEPLL_CFG);	
	clkdiv2 = __raw_readl(rDIV_CFG2);
	clkdiv2 &= ~((0x3<<24)| (0x7<<26));
	clkdiv2 |= (0x2<<24)| (0x1<<26);
	__raw_writel(clkdiv2, rDIV_CFG2);
	
	writel(((0x1<<26)| (0x1<<24)|(105<<16)|847), imapx200_i2s.regs + rI2S_PRESCALER);
	}
	if (freq == 44100){
	imapx200_get_pll((0x1<<31| 0x1<<12 | 0x00<<7 | 0x23), 12000000);
//	__raw_writel((0x1<<31 |0x1<<12| 0x0<<7 | 0x23), rEPLL_CFG);	
	clkdiv2 = __raw_readl(rDIV_CFG2);
	clkdiv2 &= ~((0x3<<24)| (0x7<<26));
	clkdiv2 |= (0x2<<24)| (0x1<<26);
	__raw_writel(clkdiv2, rDIV_CFG2);
//	writel(((0x1<<26)| (0x1<<24)|(18<<16)|151), imapx200_i2s.regs + rI2S_PRESCALER);
