...
{
	int ret, rGPPCON_VALUE, rGPPDAT_VALUE;
	imapx200_snd_device = platform_device_alloc("soc-audio", -1);
#ifdef CONFIG_IMAP_PRODUCTION_P1011A 
	rGPPCON_VALUE = __raw_readl(rGPPCON);
	rGPPCON_VALUE &= ~(0x3<<18);  //used for audio codec v3.3
	rGPPCON_VALUE |= (0x1<<18);
	__raw_writel(rGPPCON_VALUE, rGPPCON);
	rGPPDAT_VALUE = __raw_readl(rGPPDAT);
	rGPPDAT_VALUE |= (0x1<<9);
	__raw_writel(rGPPDAT_VALUE, rGPPDAT);
#elif CONFIG_IMAP_PRODUCTION_P1011B
	rGPPCON_VALUE = __raw_readl(rGPPCON);
	rGPPCON_VALUE &= ~(0x3<<14);  //used for audio codec v3.3
	rGPPCON_VALUE |= (0x1<<14);
	__raw_writel(rGPPCON_VALUE, rGPPCON);
	rGPPDAT_VALUE = __raw_readl(rGPPDAT);
	rGPPDAT_VALUE |= (0x1<<7);
	__raw_writel(rGPPDAT_VALUE, rGPPDAT);
