...
static int ads7843_pendown_state(void)
{
	...
	return !((__raw_readl(rGPGDAT) >> 5) & 1);      /* Touchscreen PENIRQ, EINT5 */	
#elif CONFIG_IMAP_PRODUCTION_P1011B
	return !((__raw_readl(rGPGDAT) >> 1) & 1);      /* Touchscreen PENIRQ, EINT1 */
#elif CONFIG_IMAP_PRODUCTION_P1011C
	return !((__raw_readl(rGPGDAT) >> 1) & 1);      /* Touchscreen PENIRQ, EINT1 */
#endif//CONFIG_IMAP_PRODUCTION_P1011C
#elif CONFIG_FB_IMAP_LCD800X600
#ifdef CONFIG_IMAP_PRODUCTION_P0811A
	return !((__raw_readl(rGPGDAT) >> 1) & 1);      /* Touchscreen PENIRQ, EINT1 */
#elif CONFIG_IMAP_PRODUCTION_P0811B
	return !((__raw_readl(rGPGDAT) >> 1) & 1);      /* Touchscreen PENIRQ, EINT1 */
#elif CONFIG_IMAP_PRODUCTION_P0811C
	return !((__raw_readl(rGPGDAT) >> 1) & 1);      /* Touchscreen PENIRQ, EINT1 */
#endif//IMAP_PRODUCTION_P0811C
#endif//CONFIG_FB_IMAP_LCD800X600
}
...
#elif CONFIG_FB_IMAP_LCD800X600
	.pressure_max           = 1000,//15000,1000(infoTM)
#endif
	.swap_xy		= 1,
...
static void __init imapx200_add_device_ts(void)
{
	...
	tmp |= (0xff<<8);
	__raw_writel(tmp, rEINTFLTCON0);
#endif//CONFIG_IMAP_PRODUCTION_P0811C
#endif//CONFIG_FB_IMAP_LCD800X600
}
