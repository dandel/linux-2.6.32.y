...
#define IMAP_BAT_FULL 570
#define IMAP_BAT_CRITICAL 470
#elif CONFIG_IMAP_PRODUCTION_P0811B
#define IMAP_BAT_FULL 610
#define IMAP_BAT_CRITICAL 500
#else
#endif
...
static int batt, old_batt = 610;
static int charging_status = 0;
static int ischargingfull(void)
{
	unsigned long tmp;
	tmp = (__raw_readl(rGPODAT)) & 0x100;// 256 charging, 256 not charging
       	return tmp; 
}
...
static int android_bat_get_capacity(union power_supply_propval *val)
{
	...
	if (ischargingfull()&&isacon()){
		...
		if (tmp_batt < IMAP_BAT_CRITICAL+15){
			val->intval = 0;
...
{	batt = battery_val;
	//printk("**batt is %d\n", batt);
	if (isacon()&&ischargingfull()){ //adaptor is connected
		if (batt > IMAP_ADAPTER){
			old_batt = batt;
	...
	else if ((old_batt > batt) && (battery_state == BATT_AGAIN)){
		old_batt = batt;
		power_supply_changed(&android_bat);	
		battery_state = BATT_ON;
	}
	...
	else if ((old_batt > IMAP_ADAPTER) || (batt > IMAP_ADAPTER)){
