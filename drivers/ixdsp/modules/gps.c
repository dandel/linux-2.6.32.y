...
static int __kread_file(const char *s, uint8_t *buf, uint32_t len)
{
	...
	for(;;)
	{
		...
		if(ret < read_len)
		{
			printk(KERN_WARNING "%s: file end before demonded bytes.\n",
			   __func__);
			break;
		}
...
void ix_gps_hw_init(int en)
{
	...
	if(en)
	{
		...
		writel(readl(rGPODAT) | (1 << 7), rGPODAT);
	} else {
...
static int ix_gps_probe(struct platform_device *pdev)
{
	 ...
	 */
#if 1
	ix_gps.auxdata = dmam_alloc_coherent(&pdev->dev, 0x2000,
...
__err_exit_1:
	dmam_free_coherent(&pdev->dev, 0x2000, ix_gps.auxdata, ix_gps.auxdata_dma);
	return err;
}
