...
//		 __func__, ep->bEndpointAddress, req->length);
...
static void __ep0_setup(void)
{
	...
	*(((uint32_t *)&crq) + 1) = readl(__reg + USB_STR1);  
	...
	dprintk(0, "r%x\n", crq.bRequest);
	dprintk(2, "r%xrt%xv%xl%xi%x\n", crq.bRequest, crq.bRequestType,
	   crq.wValue, crq.wLength, crq.wIndex);
	dprintk(0, "bRequestType=%x bRequeset=%x wValue=%x "
	   "wIndex=%x, wLength=%x\n",
...
static void __ep_rx(int ep_num)
{
	...
	if(!req)
	{
		dprintk(0, "Rx: no request in queue\n");
		return ;
	}
	/* begin transfer */
	...
	dprintk(5, "Rx, size=%x, max=%x, ep=%d, PRIR=%08x\n",
	...
	while(1) {
		...
		{
			dprintk(1, "udc detect: dma finished.\n");
			break;
		}
	}
	...
	dprintk(4, "udc_rx: size=%x, act=%x, len=%x\n",
...
static void __ep_tx(int ep_num)
{
	...
	*(uint32_t *)(d_buffer + 0x38) = 0x00000000;
	...
	dprintk(4, "Tx, len:%x act:%x, max:%x, size=%x, ep=%d\n",
	...
	dprintk(0, "req=%p, req.buf=%p, req.dma=%x, req.length=%x, req.actual=%x\n",
	   req, req->buf, req->dma, req->length, req->actual);
	writel(req->dma, __reg + USB_MDAR);
...
static void ix_udc_en(int en)
{
	if(en)
	{
		...
		dprintk(1, "rDIV_CFG2: 0x%08x\n", readl(rDIV_CFG2));
//		writel(readl(rSCLK_MASK) | 0x2030, rSCLK_MASK);
		dprintk(1, "rSCLK_MASK: 0x%08x\n", readl(rSCLK_MASK));
		...
		dprintk(1, "rPAD_CFG: 0x%08x\n", readl(rPAD_CFG));
		dprintk(1, "rUSB_SRST: 0x%08x\n", readl(rUSB_SRST));
