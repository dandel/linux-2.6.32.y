...
static struct uart_driver imapx200_uart_drv = {
	.owner		= THIS_MODULE,
	.dev_name	= "imapx200_serial",
	.nr			= 4,
	.cons		= IMAP_SERIAL_CONSOLE,
	.driver_name	= IMAPX200_SERIAL_NAME,
...
{
	writel((readl(rGPACON) & ~(0xf)) | 0x0a, rGPACON);
	writel((readl(rGPACON) & ~(0xf<<8)) | (0x0a<<8), rGPACON);
	writel((readl(rGPBCON) & ~(0xf)) | (0x0a), rGPBCON);
	writel((readl(rGPBCON) & ~(0xfff)) | (0xaaa), rGPBCON);
}
