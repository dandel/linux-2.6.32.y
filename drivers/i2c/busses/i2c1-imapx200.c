...
static int imapx200_i2c_init_register(struct imapx200_i2c *i2c)
{
	...
	if(i2c->dirty_bit == 1)
	{
		...
		__raw_writel((IMAPX200_IIC_DEFAULT_SDA & 0xff), i2c->base_reg + rIC_IGNORE_ACK1);
...
/*
 *imapx200_i2c_write_poll
 */
static void imapx200_i2c_write_poll(struct imapx200_i2c *i2c,
					struct i2c_msg *msg)
{
	unsigned int i;
	unsigned char byte;
				      ...
				      struct i2c_msg *msg)
{
	...
	if(i2c->state == IIC_CUR_IDLE)
	{
		for(i=0;i<msg_num;i++)	
		{
			...
			{
				i2c->state = IIC_CUR_IDLE;
				printk("functions[%s] iic1 data transfer timeout slave address is %x\n",__func__,i2c->msg->addr);
			}
		}
	}
